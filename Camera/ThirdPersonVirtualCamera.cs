...
public partial class ThirdPersonVirtualCamera : VirtualCamera
{
    ...
    private static float zoom_perc = 0.5f;
    ...
    public override Transform3D CalculateTransform()
    {
        ...
        transform = TransformCollision(transform);
        return transform;
    }
    ...
    private Transform3D TransformCollision(Transform3D transform)
    {
        var extra_length = 0.01f;
        var dir_to_origin = transform.Origin - Raycast.GlobalPosition;
        Raycast.GlobalPosition = FollowTarget.GlobalPosition;
        Raycast.TargetPosition = dir_to_origin + dir_to_origin.Normalized() * extra_length;
        if (Raycast.IsColliding())
        {
            ...
            if (dir_to_point_extended.Length() < dir_to_origin.Length())
            {
                var dir = point - transform.Origin;
                var offset = dir.Normalized() * extra_length;
                return transform.Translated(dir + offset);
            }
        }
